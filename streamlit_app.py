...
#   miles_needed_per_day = user_inputs['annual_mileage'] / (user_inputs['num_charging_days_per_week']*52)
#   charging_time_hours = miles_needed_per_day / (user_inputs['miles_per_kwh'] * user_inputs['charger_power_rating'])

#   # Display calculated values
#   st.subheader("Charging goals based on the input EV owner profile and charging behaviour:")

#   st.write(f"* Average charging miles needed per session: **{miles_needed_per_day:.2f} miles**")
#   st.write(f"* Charging time duration: **{charging_time_hours:.2f} hours**")
#   st.markdown('---')
...
#   transformed_dfs = transform_df(df, charging_time_hours)
#   st.write("An example dataframe from the transformed_dfs list of all the daily dfs")
#   st.dataframe(transformed_dfs[10])

  
#   final_df_with_results = get_daily_recommended_and_baseline_emissions(transformed_dfs, user_inputs, charging_time_hours)
#   st.write("Dataframe detailing the baseline charging and recommended charging emissions for every day of the year")
#   st.dataframe(final_df_with_results)
#   st.markdown('---')
...
#   annual_emissions_recommended_charging = final_df_with_results['total_emissions_recommended_charging'].mean() * multiplication_factor
#   annual_emissions_baseline_charging = final_df_with_results['total_emissions_baseline_charging'].mean() * multiplication_factor
...
#   st.write(f"* Total annual emisisons associated with Baseline Charging Approach: : **{annual_emissions_baseline_charging:.2f} lb of CO2**")
#   st.write(f"* Total annual emisisons associated with Recommended Charging Approach: : **{annual_emissions_recommended_charging:.2f} lb of CO2**")
#   st.write(f"* Total annual emisisons avoided: : **{annual_emissions_avoided:.2f} lb of CO2**")
...
#   with st.expander('Visualize the data transformations'):
...
#     dates = [df.index[0].date() for df in transformed_dfs]  # Get the list of dates from the DataFrames

#     # Create a date input for user to select a date
#     selected_date = st.date_input("Select a date:", value=pd.to_datetime(transformed_dfs[11].index[0]).date())

#     if st.button("Observe the Transformed Dataframe"):
#         if selected_date in dates:
#             # Find the index of the selected date
#             index_of_selected_date = dates.index(selected_date)
#             # Get the corresponding DataFrame
#             selected_transformed_df = transformed_dfs[index_of_selected_date]

#             # Now you can use selected_df for visualization
#             st.dataframe(selected_transformed_df)  # Display the DataFrame or pass it to your plotting function
